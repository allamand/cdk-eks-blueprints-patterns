...
export default class MultiRegionConstruct {
  async buildAsync(scope: Construct, id: string): Promise<blueprints.EksBlueprint[]> {
    ...
    const east1 = await blueprint.clone('us-east-1').addOns(devBootstrapArgo).buildAsync(scope, `${id}-us-east-1`);

    const east2 = await blueprint.clone('us-east-2').addOns(testBootstrapArgo).buildAsync(scope, `${id}-us-east-2`);

    const west2 = await blueprint.clone('us-west-2').addOns(prodBootstrapArgo).buildAsync(scope, `${id}-us-west-2`);
...
async function prevalidateSecrets() {
  try {
    ...
    throw new Error(
      'Both github-ssh-key and argo-admin-secret secrets must be setup for the multi-region pattern to work.',
    );
  }
}
